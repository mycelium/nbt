...
public class AnalistController {
		...
		if(_crDao.findOne(changeRequestId)!=null)
			 {
				...
				_crDao.addCR(updatedCR);
			 }
		
		return "redirect:/site/analist";
	}
	
	@RequestMapping(value = "/issue/edit", method = RequestMethod.POST)
	public String editIssue( HttpServletRequest request)
	{
		IssueEntity updatedIssue;
		String issueId=request.getParameter("issId");
		if(_issueDao.findOne(issueId)!=null)
			 {
				updatedIssue=_issueDao.findOne(issueId);
				updatedIssue.setCaption(request.getParameter("issueCaption"));
				updatedIssue.setDescription(request.getParameter("issueDescription"));
				_issueDao.addIssue(updatedIssue);
			 }
		return "redirect:/site/analist";
	...
	public void delIssueFromCR(/*HttpServletRequest request*/
		...
		String[] idOfIssues = iac.getIdIssueList();
		if ( idOfIssues!= null) {
			_crDao.deleteIssueFromCR(idOfCR[0], idOfIssues);
			for (String idOfIssue : idOfIssues)
				_issueDao.deleteCRFromIssue(idOfIssue, idOfCR[0]);
		}
